...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center navbar">
     <img src={logo} className="w-[124px] h-[32px] cursor-pointer"/>

     <ul className="list-none sm:flex hidden justify-end items-center flex-1">

      {navLinks.map((nav,index)=>(

        <li key={nav.id} className={`font-poppins font-normal z-[10] cursor-pointer text-[16px] text-white ${index===navLinks.length-1?'mr-0':'mr-10'}`} >
